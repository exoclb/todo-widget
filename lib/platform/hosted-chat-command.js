...
function createDashboardCommandLogEntry(entry) {
  return {
    id: entry.id,
    commandName: entry.commandName,
    rawCommandText: entry.rawCommandText,
    actorLabel: entry.actorLabel,
    outcome: entry.outcome,
    ignoredReason: entry.ignoredReason ?? null,
    createdTaskId: entry.createdTaskId ?? null,
    affectedTaskId: entry.affectedTaskId ?? null,
    createdAt: entry.createdAt ?? null,
  };
}

export async function listDashboardCommandLog(repository, input) {
  const entries = await repository.listCommandLogs(input.streamerProfileId, input.widgetId, input.limit ?? 25);
  return entries.map(createDashboardCommandLogEntry);
}
